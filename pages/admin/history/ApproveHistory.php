...
<html>
<head>
	<title>View Approve History</title>
        ...
        <form class="searchbar" method="post">
            <table class="searchcontainer">
                <tr class="searchcontainer">
                    <th class="searchcontainerrequest"><h2>Approve History</h2></th>
...
if(isset($_POST["search"])){
    ...
    $sql = "SELECT * FROM request WHERE Request_Status='Approve' AND Request_Id='".$search."'";
    $result = $conn->query($sql);

}else if(isset($_POST["showall"])){
    $sql = "SELECT * FROM request WHERE Request_Status='Approve'";
    $result = $conn->query($sql);
}else{
    $sql = "SELECT * FROM request WHERE Request_Status='Approve'";
    $result = $conn->query($sql);
